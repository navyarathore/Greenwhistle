...
export default class InteractionManager {
  ...
  private interactionZones: Map<
    string,
    { x: number; y: number; width: number; height: number; type: InteractionType }
  > = new Map();
  ...
  /**
   * Update interaction effects and animations
   */
  private updateInteractionEffects(time: number, delta: number): void {
    // Update any active interaction effects
    // This could include tool swing animations, particle effects, etc.

    // Update cooldowns for interactions
    this.interactionCooldowns.forEach((cooldownTime, interactionId) => {
  ...
  private isInteractionOnCooldown(interactionId: string): boolean {
    return this.interactionCooldowns.has(interactionId);
  }
   ...
   */
  private createToolUseEffect(toolId: string, position: { x: number; y: number }): void {
    // Convert tile position to world coordinates
